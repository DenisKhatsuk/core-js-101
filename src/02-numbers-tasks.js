...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const cosOfAngle = dotProduct / (magnitude1 * magnitude2);
...
function getLastDigit(value) {
  ...
  return stringFromValue[stringFromValue.length - 1];
}
...
function roundToPowerOfTen(num, pow) {
  if (pow === 0) return num;
  const power = 10 ** pow;
  const numToRound = num / power;
  return Math.round(numToRound) * power;
}
...
function toNumber(value, def) {
  if (value === null || !Number(value)) return def;
